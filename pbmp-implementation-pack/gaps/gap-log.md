...
| GAP-REQ-SALES-001-05 | REQ-SALES-001 | Missing Acceptance Criteria | Structured action includes `send_notification` to Sales Manager, but approved AC list includes only task creation. What is the approved AC ID and expected assertion for notification behavior? Implementation includes notification per structured_actions; test coverage added pending formal AC approval. | Open | Development | Business Analyst |
